...
void *thread_job(void *arg) {
  ...
  cout << "Thread " << thread_num << " is running with param1: " << param1
       << " and param2: " << param2 << endl;

  // Вывод информации о размере стека
  size_t stack_size;
  pthread_attr_getstacksize(attr, &stack_size);
  cout << "Thread " << thread_num << " stack size: " << stack_size << " bytes"
       << endl;

  // Вывод информации о политике планирования
  int sched_policy;
  pthread_attr_getschedpolicy(attr, &sched_policy);
  cout << "Thread " << thread_num << " scheduling policy: " << sched_policy
       << endl;
  ...
  cout << "Thread " << thread_num
       << " scheduling priority: " << sched_param.sched_priority << endl;

  // Вывод информации о наследуемости планирования
  int inheritsched;
  pthread_attr_getinheritsched(attr, &inheritsched);
  cout << "Thread " << thread_num << " inherits scheduling: " << inheritsched
       << endl;
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 4) {
    cerr << "Usage: " << argv[0] << " <number_of_operations> <param1> <param2>"
         << endl;
    return EXIT_FAILURE;
  }
  ...
  for (int i = 0; i < n; ++i) {
    ...
    thread_args[0][i] = {i + 1, stod(argv[2]) + i, stod(argv[3]) + i,
                         &attr}; // thread_num, param1, param2, attr

    if (pthread_create(&threads[0][i], &attr, thread_job, &thread_args[0][i]) !=
        0) {
    ...
    cout << "Outer Time for thread " << i + 1 << ": " << outer_time << " us"
         << endl;
    cout << "Thread creation cost: " << outer_time - inner_time << " us"
         << endl;
    cout << "=================================================================="
         << endl;
